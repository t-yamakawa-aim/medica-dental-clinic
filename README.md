...
- **最終更新**: 2026-08-13（「新着情報（お知らせ）」「ブログ」の投稿型ページ群を追加：一覧・詳細・年別/月別アーカイブ・カテゴリ別一覧・ページネーションをCloudflare D1のSQLで実装。加えて、ブログ/お知らせへの写真・リンク・ボタンの掲載機能を追加：一覧サムネイル・詳細ページアイキャッチは横長16:9でPC/スマホ自動レスポンシブ、本文中にも`![](img)` / `[](url)` / `[](url){button}`記法で複数枚の写真・リンク・ボタンを挿入可能）
