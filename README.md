...
※ 下層ページ（`/medical`, `/service`, `/symptoms`, `/recruit`, `/news`, `/news/:id`, `/blog`, `/blog/:id`, `/contact`, `/privacy`）は現在ヘッダー・フッターのリンク先として用意されていますが、ページ自体は未実装です（今後、下層ページの情報提供後に実装予定）。
...
1. **下層ページの実装**（ユーザーからの情報提供待ち）
   - `/medical`（当院について：私たちの目指すもの、院長紹介、当院概要、施設・設備紹介、スタッフ紹介）
